...
-(void)setupUI {
    ...
    registerBtn.backgroundColor = CNavBgColor;
    ...
    ViewRadius(registerBtn, 22);
    [registerBtn addTarget:self action:@selector(registerAction) forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:registerBtn];
    [registerBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(passwordLine.mas_bottom).offset(kAdaptedHeight(40));
        make.centerX.equalTo(self.view.mas_centerX);
    ...
    agreementLabel.text = @"注册即表示您同意《现金贷管家用户使用协议》";
    agreementLabel.textColor = CHexColor(@"999999");
    agreementLabel.font = kAdaptedFontSize(kAdaptedWidth(14));
    [self.view addSubview:agreementLabel];
    [agreementLabel sizeToFit];
    [agreementLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerX.equalTo(self.view.mas_centerX);
        make.bottom.equalTo(self.view.mas_bottom).offset(-kAdaptedHeight(40));
    }];
    
    [self label:agreementLabel editStringColor:@"注册即表示您同意《现金贷管家用户使用协议》" editStr:@"《现金贷管家用户使用协议》" color:CHexColor(@"ec761f")];
    
    agreementLabel.userInteractionEnabled = YES;
    [agreementLabel addGestureRecognizer:[[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(lookAgreementAction)]];
    
    
}
- (void)label:(UILabel *)label editStringColor:(NSString *)string editStr:(NSString *)editStr color:(UIColor *)color {
...
-(void)registerAction {
    ...
    NSDictionary *params = @{@"phone":self.phoneTf.text,@"vercode":self.codeTf.text,@"password":self.passwordTf.text};
    [FXHttpTool postWithURL:@"regiUser" params:params sucess:^(id json) {
...
#pragma mark ---- 查看注册协议
-(void)lookAgreementAction {
    DLog(@"查看注册协议");
    
    NSString *urlString = NSStringFormat(@"http://passpay.kouzicr.com/Home/Login/protocol");
...
-(UITextField *)passwordTf {
    if (!_passwordTf) {
        ...
        _passwordTf.placeholder = @"请输入密码，6-12位字符";
