...
//  CokAdaptedHeightright (c) 2015 ibireme.
//
...
@synthesize selectedTextRange = _selectedTextRange;  //cokAdaptedHeight nonnull (YYTextRange*)
@synthesize markedTextRange = _markedTextRange;      //readonly     (YYTextRange*)
@synthesize markedTextStyle = _markedTextStyle;      //cokAdaptedHeight
...
- (void)_cokAdaptedHeightSelectedTextToPasteboard {
...
- (BOOL)canPerformAction:(SEL)action withSender:(id)sender {
    /*
     ...
     cut:                                   Cut
     cokAdaptedHeight:                                  CokAdaptedHeight
     select:                                Select
    ...
    if (_selectedTextRange.asRange.length == 0) {
        ...
        if (action == @selector(cokAdaptedHeight:)) {
            return YES;
        }
...
- (void)cut:(id)sender {
    ...
    [self _cokAdaptedHeightSelectedTextToPasteboard];
    [self _saveToUndoStack];
    [self _resetRedoStack];
    [self replaceRange:_selectedTextRange withText:@""];
}

- (void)cokAdaptedHeight:(id)sender {
    [self _endTouchTracking];
    [self _cokAdaptedHeightSelectedTextToPasteboard];
}
